...
* You're handling errors with Try/Catch, and you made it to the catch block! You reference ```$Error[0]```. Turns out by some fluke another error squeeked its way in, pushing the error you care about to ```$Error[1]```. Always refer to the current error in the catch block as ```$_```. Do not rely on ```$Error```.
