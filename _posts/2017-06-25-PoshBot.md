...
* Interesting balance between creating a web interface or GUI for a PowerShell script, and forcing folks to use PowerShell commands
* Share gifs
...
* Add a new [bot](https://api.slack.com/bot-users) integration:
...
A PoshBot plugin is basically a PowerShell module with a few PoshBot bits.  We'll skip [the typical module scaffolding](http://ramblingcookiemonster.github.io/Building-A-PowerShell-Module/) to keep our example simple.
...
There's so much you can do.  Many folks start with read-only commands until they get comfortable with things.  Among many other tasks, you might...
...
* Get stuff and do stuff from a variety of other systems!
  ...
  * IPAM systems
  * Monitoring systems
  * Databases - e.g. Query a database with read-only access
  * etc.
...
Consider read-only accounts, or delegating only the privileges needed for any approach you take, and consider classifying your system as appropriate to ensure appropriate security controls are in place : )
...
There's plenty of other reading material out there, consider checking these out:
