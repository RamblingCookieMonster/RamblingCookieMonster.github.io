...
I grew up using IRC.  Before Napster and Steam, there were other ways to find music and games.  As time went by, I shifted over to more legitimate use, with freenode and other networks offering a variety of solid technical resources, including channels for PowerShell, VMware, Citrix, and more.
...
So! Let's step back a moment and consider why you might want an asynchronous group chat system...  Let's use two example cases.  Which would you prefer:
...
Oh. And if you go beyond sending messages to your chat system, you can do some crazy fun things with a bot. Check out this awesome bit from Matt Hodge on setting up [Hubot with PowerShell](https://hodgkins.io/chatops-on-windows-with-hubot-and-powershell)
...
* Maybe the existing integration doesn't offer enough customization.  SCOM is a great example.  Rather than sending [pretty HTML e-mails](https://gallery.technet.microsoft.com/scriptcenter/PowerShell-HTML-Notificatio-e1c5759d), you could add context to SCOM alerts and send them along to Slack

Wait.  But why would I send my stuff to Slack?  It turns out that this is quite helpful.  Who wouldn't want to have informational logs, various levels of alerts, and other conversation, all in one spot, search-able from any client, without worrying about it hitting you junk mail, being archived, or being spread across your various logging, monitoring, mail, file systems, event logs, and other sources that usually house this data?  Sure, use those other solutions, but do consider how helpful it is to send this stuff to a chat system.

But... this is a PowerShell post, where's the PowerShell?
...
I some work and ideas from jgigler and Steven, and put together a quick and dirty [PSSlack module](https://github.com/RamblingCookieMonster/PSSlack)!  Let's do a quick walk through.
...
We [don't support](https://github.com/RamblingCookieMonster/PSSlack/issues/12) sending things like gifs yet - would love a pull request : )
