...
We just walked through two example deployment types. What if you want more deployment options?
...
* In your deployment script (SCP.ps1), include a 'Deployment' parameter
  * See [FilesystemRemote.ps1](https://github.com/RamblingCookieMonster/PSDeploy/blob/master/PSDeploy/PSDeployScripts/FilesystemRemote.ps1) for a full implementation example
* Include comment based help to explain the deployment and any extra parameters you accept. This is a good spot to describe YAML specifications, if you use them.
...
If you want to include options in a deployment config file, similar to the mirror option we use in Filesystem deployments, just include them in the deployments.yml.
...
Here's an example extended deployments.yml with a hostname and port option:
